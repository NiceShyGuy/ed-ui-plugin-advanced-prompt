 ...
 * Press Chat Imagine button to send the prompt to the chat gpt-3.5-turbo model by default.
 * Press Settings button to open the settings dialog. Use OpenAI's key and set your name.
 * Adjust the settings to your liking and press Save Settings.
 */

(function () {
    "use strict";
    let styleSheet = document.createElement("style");
    styleSheet.textContent = `summary { cursor: pointer; }
details { font-size: 10pt; }
.ap-table { width: 100%; }
.ap-row { display: grid; grid-template-columns: 32px auto; grid-gap: 10px; }
i { font-style: normal; }
.ap-table th, .ap-table td { padding: 0; }
.chat, .settings { height: 24px; }
...
an astronaut in a golden space suit, riding on the back of a majestic black horse, galloping on the moon towards the camera with its mane and tail flowing in the wind, with the Earth setting in the background, Photograph, Landscape, by NASA`,
    ...
    function buildAdvancedPrompt() {
        ...
        let details = document.createElement("details"), summary = document.createElement("summary");
        summary.innerHTML = `Advanced Prompt<a href="https://github.com/cmdr2/stable-diffusion-ui/wiki/Writing-prompts" target="_blank" style="text-decoration: none;"> <i class="fa-solid fa-circle-question help-btn"><span class="simple-tooltip right">Click to learn more about writing Prompts</span></i></a><small> (optional) (<span class="token-count">${countTokens(promptField.value)}</span>) tokens <button class="chat">🚀 Chat Imagine</button><button class="settings"><span id="settings">⚙️</span></button></small>`;
        details.appendChild(summary), details.appendChild(table), apControl.appendChild(details);
